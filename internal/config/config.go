...
type Config struct {
	...
	AMQPURL           string
	QueueName         string
	AMQPMsgMaxRetries int
	AMQPMsgRetryDelay time.Duration
	Mode              string
...
func Load() (Config, error) {
	cfg := Config{
		...
		AMQPMsgMaxRetries: getEnvInt("AMQP_MESSAGE_MAX_RETRIES", 3),
		AMQPMsgRetryDelay: time.Duration(getEnvInt("AMQP_MESSAGE_RETRY_DELAY_MS", 2000)) * time.Millisecond,
	...
	if cfg.AMQPMsgMaxRetries < 0 {
		return Config{}, fmt.Errorf("AMQP_MESSAGE_MAX_RETRIES must be >= 0")
	}
	if cfg.AMQPMsgRetryDelay <= 0 {
		return Config{}, fmt.Errorf("AMQP_MESSAGE_RETRY_DELAY_MS must be > 0")
	}
